...
if __name__ == "__main__":
    ...
    # Создаем экземпляр GradientWidget вместо QWidget
    window = GradientWidget()
    window.setGeometry(100, 100, 640, 620)  # Увеличил размер окна
    ...
    palette.setColor(QPalette.Button, base_color)  # Цвет кнопок
    palette.setColor(QPalette.ButtonText, QColor(255, 255, 255))  # Цвет текста на кнопках
    ...
    # Цвет текста в полях ввода и кнопках
    ...
    auth_button.setStyleSheet(text_input_style)  # Применяем стиль к кнопке
    ...
    time_units.setStyleSheet(text_input_style)  # Применяем стиль к выпадающему списку
    ...
    sprint_combo_box.setStyleSheet(text_input_style)  # Применяем стиль к выпадающему списку
    ...
    parent_task_combo_box.setStyleSheet(text_input_style)  # Применяем стиль к выпадающему списку
    ...
    create_task_button.setStyleSheet(text_input_style)  # Применяем стиль к кнопке
    due_date_checkbox.stateChanged.connect(
        lambda state: due_date_edit.setDisabled(state))

    task_link_label = QLabel("", window)  # Добавленный QLabel
    ...
    image_label.setGeometry(580, 5, 100, 100)  # Установите размеры и положение по вашему усмотрению 
    image_label.setPixmap(pixmap)
    app_icon = QIcon("C:/Users/1/Desktop/scripts/vizard/win/omni.ico")  # Замените "path/to/your/icon.png" на путь к вашей иконке
